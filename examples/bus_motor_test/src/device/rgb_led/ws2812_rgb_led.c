...
/**
 * @brief ws2812_rgb_led_fill 内部辅助函数
 * @param r 参数值
 * @param g 参数值
 * @param b 参数值
 * @return 状态码
...
RgbLedStatus ws2812_rgb_led_make_config(RgbLedConfig* out_config, const RgbLedPortOps* ops,
    ...
    if(out_config == 0 || ops == 0 || ops->write == 0 || color_buffer == 0 || tx_buffer == 0)
        return RGB_LED_STATUS_INVALID_PARAM;

    out_config->ops = ops;
    out_config->pixel_count = WS2812_RGB_LED_DEFAULT_PIXEL_COUNT;
    ...
    out_config->reset_bytes = WS2812_RGB_LED_DEFAULT_RESET_BYTES;
